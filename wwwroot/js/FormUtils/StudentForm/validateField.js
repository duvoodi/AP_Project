...
function validateField(inputId, showError, AllowOptionalFieldsInEdit = false) {
  ...
  const requiredFields = ['FirstName', 'LastName', 'Email', 'EnrollmentYear', 'Password', 'ConfirmPassword'];
  ...
  case 'StudentId':
    if (!validateField('EnrollmentYear', false)) { // اول بررسی معتبر بودن سال استخدام بدون نمایش ارور
    ...
    const enteredEnrollmentYear = document.getElementById('EnrollmentYear')?.value;
    // امکان نداارد کد موارد زیر باشد
    if (!isValidNumeric(value) || value.length !== 9) {
      // ورودی غیر مجاز را پاک میکنیم
      input.value = '';
      replaceError_IfAllowed(inputId, 'خطا در دریافت کد دانشجویی');
      return false;
    }
    const prefixExpected = enteredEnrollmentYear.substring(1, 4);
  ...
  case 'EnrollmentYear':
    ...
    if (parsedHireYear < 1390 || parsedHireYear > currentPersianYear + 1) { // تا سال بعدی رو میتواند زودتر وارد کند
      replaceError_IfAllowed(inputId, 'ورودی خارج از بازه مجاز است');
      return false;
    }
    break;
