function validateField(inputId, showError) {
  ...
  const requiredFields = ['Id', 'Building', 'RoomNumber', 'Capacity'];
  ...
  if (inputId.includes('Building')) {
    ...
    // 3.3. بررسی فرمت صحیح نام ساختمان
    if (!isValidName(value)) {
      replaceError_IfAllowed('ورودی نامعتبر است');
      return false;
    }
  } 
  else if (inputId.includes('RoomNumber') || inputId.includes('Capacity')) {
    // 3.1. بررسی خالی بودن
    if (!value) {
      replaceError_IfAllowed('لطفاً این فیلد را پر کنید');
      return false;
