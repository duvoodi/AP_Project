...
namespace AP_Project.FormViewModels.StudentForm
{
    ...
    public static class StudentFormViewModelExtensions
    {
        // تابع اعتبارسنجی یک فیلد بر اساس پراپرتی مدل
        public static bool ValidateField(this ModelStateDictionary modelState, StudentFormViewModel model, string propertyName, bool showError = true, bool AllowOptionalFieldsInEdit = false)
        {
            
            var propInfo = typeof(StudentFormViewModel).GetProperty(propertyName);
            ...
            switch (propertyName)
            {
                ...
                // چک کد دانشجویی (StudentId)
                case nameof(model.StudentId):
                    if (!modelState.ValidateField(model, nameof(model.EnrollmentYear), false))
                    {
                        ReplaceError_IfAllowed(propertyName, "");
                        propInfo?.SetValue(model, "");
                        return false;
                    }
                    var enteredEnrollmentYear = model.EnrollmentYear;
                    if (!value.IsValidNumeric() || value.Length != 9)
                    {
                        propInfo?.SetValue(model, "");
                        ReplaceError_IfAllowed(propertyName, "خطا در دریافت کد دانشجویی");
                        return false;
                    }
                    var prefixExpected = enteredEnrollmentYear.Substring(1, 3);
                    var prefixActual = value.Substring(0, 3);
                    var mid = int.Parse(value[3].ToString());

                    if (prefixActual != prefixExpected || mid < 0 || mid > 4)
                    {
                        propInfo?.SetValue(model, "");
                        ReplaceError_IfAllowed(propertyName, "خطا در دریافت کد دانشجویی");
                        return false;
                ...
                // چک سال ورودی
                case nameof(model.EnrollmentYear):
                    ...
                    var parsedEnrollmentYear = int.Parse(value);
                    var pc = new PersianCalendar();
                    int currentPersianYear = pc.GetYear(DateTime.Now);
                    if (parsedEnrollmentYear < 1390 || parsedEnrollmentYear > currentPersianYear + 1) // تا سال بعدی رو میتواند زودتر وارد کند
                    {
                        ReplaceError_IfAllowed(propertyName, "ورودی خارج از بازه مجاز است");
                        return false;
                    }
